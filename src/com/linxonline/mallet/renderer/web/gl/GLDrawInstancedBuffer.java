...
public class GLDrawInstancedBuffer extends GLBuffer
{
	...
	public void draw( final Matrix4 _projection )
	{
...

		{
			...
			for( int i = 0; i < size; ++i )
			{
				...
				if( name.equals( storageName ) == true ) 
				{
					...
					//MGL.bindBufferBase( MGL.GL_SHADER_STORAGE_BUFFER, i, glTransStorage.id[0] ) ;
				}
				else
				{
		...
		MGL.drawElementsInstanced( style, indexCount, MGL.GL_UNSIGNED_INT, 0L, transformations.drawCount ) ;
	...
	/*private void uploadInstanceToVRAM()
	{
		...
		MGL.bufferData( MGL.GL_ELEMENT_ARRAY_BUFFER, indiciesLengthBytes, indexBuffer, MGL.GL_DYNAMIC_DRAW ) ;
		MGL.bufferData( MGL.GL_ARRAY_BUFFER, verticiesLengthBytes, vertexBuffer, MGL.GL_DYNAMIC_DRAW ) ;
	}
