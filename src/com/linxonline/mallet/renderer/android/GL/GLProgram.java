...
public class GLProgram extends Resource
{
	public enum DataType
	{
		...
		{
			public void load( final ProgramData _data, final int _index )
			{
		...
		SAMPLER2D( new UniformDelegate()
		{
			...
			public void load( final ProgramData _data, final int _index )
			{
				...
				GLES30.glUniform1i( inUniform, textureUnit ) ;
				GLES30.glActiveTexture( GLES30.GL_TEXTURE0 + textureUnit ) ;				//GLRenderer.handleError( "Activate Texture", _gl ) ;
				GLES30.glBindTexture( GLES30.GL_TEXTURE_2D, program.textureID[textureUnit] ) ;		//GLRenderer.handleError( "Bind Texture", _gl ) ;
				textureUnit += 1 ;
			}
